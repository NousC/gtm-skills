...
| Skill | Use it for |
|-------|------------|
| [`linkedin-engagers`](.claude/skills/linkedin-engagers/SKILL.md) | Turn a creator's recent engagers into an ICP-scored Nous lead list, every engagement recorded as a signal on the record |
...
| [`sales-nav-builder`](.claude/skills/sales-nav-builder/SKILL.md) | Turn a LinkedIn Sales Navigator search into a complete, ICP-scored lead list, paying for emails only on the leads worth contacting. Claude builds the search from structural filters first, Evaboot extracts every lead with no emails, Nous imports all of them and scores each against your ICP, then Prospeo finds the verified email from the LinkedIn URL on qualified net-new leads. Every lead stays in the list, labelled by its ICP score. For Sales Nav users |
